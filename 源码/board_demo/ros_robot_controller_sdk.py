...
class PacketFunction(enum.IntEnum):
    ...
    PACKET_FUNC_PWM_SERVO = 4  # PWM舵机控制, 板子上从里到外依次为1-4(PWM control,  from the innermost to the outermost, the board are labeled as 1 to 4)
...
class PacketReportKeyEvents(enum.IntEnum):
    # 按键的不同状态(the different states of key)
...
class Board:
    ...
    def set_oled_text(self, line, text):
        data = [line, len(text)] # 子命令为 0x01 设置 SSID, 第二个字节是字符串长度，该长度包含'\0'字符串结束符(the subcommand 0x01 sets the SSID. The second byte indicates the length of the string, including the '\0' string termination character)
